...
def bulk_replace_index_root(s: str, index_root_user: str, local_root: Path) -> str:
    ...
    Uses POSIX substring rules for ``/home/...`` roots; case-insensitive replace for native Windows roots.
    """
    ...
    if is_posix_style_index_root(index_root_user):
        ...
        if irn not in n:
            return s
        return n.replace(irn, loc_alt)
    old = normalize_native_index_root(index_root_user).replace("\\", "/")
    n = s.replace("\\", "/")
    if old.lower() not in n.lower():
        return s
    return re.compile(re.escape(old), re.IGNORECASE).sub(loc_alt, n)
